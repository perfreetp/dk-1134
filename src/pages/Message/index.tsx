...
import { MessageCircle, Send, Bell, ArrowLeft } from 'lucide-react';
...
export default function MessagePage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 flex flex-col">
      <Header
        title={selectedConversation ? (otherUser?.name || '聊天') : '消息'}
          ...
          <div className="max-w-md mx-auto px-4 py-4 space-y-4">
            ...
            <div className="space-y-2">
              {conversations.map((conversation) => {
                const convOtherUser = conversation.participantUsers?.find(
                  (u) => u.id !== currentUser?.id
                );
                  ...
                  >
                    <div className="flex items-center space-x-3 p-3 bg-white hover:bg-gray-50 rounded-xl transition-colors">
                      <div className="relative">
                        <Avatar
                          src={convOtherUser?.avatar}
                          name={convOtherUser?.name || ''}
                          size="lg"
                        />
                        {hasUnread && (
